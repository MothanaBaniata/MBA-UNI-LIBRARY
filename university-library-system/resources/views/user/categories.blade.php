                    ...
                    <ul>
                        ...
                        <!-- Dropdown to select a category -->
                            ...
                            <form method="GET" action="{{ route('user.categories') }}">
                                <select class="custom-select" id="categoryDropdown" name="category" onchange="this.form.submit()">
                        ...
                        <li>
                            <form method="GET" action="{{ route('user.categories') }}">
                                <input type="text" class="form-control" name="search" placeholder="Search Books" value="{{ request('search') }}">
                            </form>
                        </li>
                    </ul>
                </div>
                        ...
                        <div class="col-lg-4 templatemo-item-col all {{ strtolower($book->category->name) }}">
                            <div class="meeting-item">
                                ...
                                <div class="down-content">
                                    <div class="date">
                                        <h6>Available <span>{{ $book->total_copies - $book->borrowed_copies }}</span></h6>
                                    </div>
                                    <a href="{{ route('user.book.show', $book->id) }}">
                                        <h4>{{ $book->title }}</h4>
                                    </a>
...
<script>
    ...
    var categoryId = selectElement.value; // Get selected category ID
    var searchTerm = document.querySelector('input[name="search"]').value; // Get search term if any

    var url = new URL(window.location.href); // Get current URL

    // Update the URL with the selected category if applicable
    if (categoryId) {
        url.searchParams.set('category', categoryId);
    } else {
        url.searchParams.delete('category'); // Remove category from URL if no category selected
    }

    // Update search term in URL if there's any
    if (searchTerm) {
